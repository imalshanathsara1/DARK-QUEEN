...
const startAtlas = async () => {
  ...
  async function installPlugin() {
    ...
    if (!plugins.length || plugins.length == 0) {
      console.log(
        chalk.redBright("No Extra Plugins Installed ! Starting SamuZa...\n")
      );
    } else {
      console.log(
      ...
      console.log(
        chalk.greenBright(
          "All Plugins Installed Successfully ! Starting SamuZa...\n"
        )
      );
    }
  }
  ...
  Atlas.ev.on("connection.update", async (update) => {
    ...
    if (connection) {
      console.info(`[ SAMUZA ] Server Status => ${connection}`);
    }
    ...
    if (connection === "close") {
      ...
      if (reason === DisconnectReason.badSession) {
        console.log(
          `[ SAMUZA ] Bad Session File, Please Delete Session and Scan Again.\n`
        );
        process.exit();
      } else if (reason === DisconnectReason.connectionClosed) {
        console.log("[ SAMUZA ] Connection closed, reconnecting....\n");
        startAtlas();
      } else if (reason === DisconnectReason.connectionLost) {
        console.log("[ SAMUZA ] Connection Lost from Server, reconnecting...\n");
        startAtlas();
      } else if (reason === DisconnectReason.connectionReplaced) {
        console.log(
          "[ SAMUZA ] Connection Replaced, Another New Session Opened, Please Close Current Session First!\n"
        );
        process.exit();
      } else if (reason === DisconnectReason.loggedOut) {
        clearState();
        console.log(
          `[ SAMUZA ] Device Logged Out, Please Delete Session and Scan Again.\n`
        );
        process.exit();
      } else if (reason === DisconnectReason.restartRequired) {
        console.log("[ SAMUZA ] Server Restarting...\n");
        startAtlas();
      } else if (reason === DisconnectReason.timedOut) {
        console.log("[ SAMUZA ] Connection Timed Out, Trying to Reconnect...\n");
        startAtlas();
      } else {
        console.log(
          `[ SAMUZA ] Server Disconnected: "It's either safe disconnect or WhatsApp Account got banned !\n"`
        );
      }
    }
    if (qr) {
